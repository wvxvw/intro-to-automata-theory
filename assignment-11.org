...
*** Answer 12
    ...
    2. For each accepting state $q_n$ directly linked from $q_0$ we do the
       ...
       + remove transitions from states $Q_i$ directly leading to $q_n$, for
    ...
    #+BEGIN_SRC latex
      \begin{algorithm}
        ...
        \begin{algorithmic}
          \Procedure {$\textit{ensure-alphabet-length-ne-one}$}{$\Sigma, Q, q_0, F, \delta$}
            \State $loops \gets \{i \;|\; \exists i \in \Sigma. (\delta(q_0, i) = q_0)\}$
            \State $new \gets \textit{make new accepting state}$
            \State $backarcs \gets \{(v, w, i) \;|\; \exists v,w \in Q. (\delta(q_0, j) = v = \delta(w, i))\}$
            \For {$input \in loops$}
